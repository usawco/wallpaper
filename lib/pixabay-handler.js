...
var theFeed = ( () => {
    let models = [];
    // disable on windows platform until I rework the caption logic in JS.
    if ( config.providers.pixabay.enabled && "win32" !== os.platoform() ) {
        _util.traceLog('Creating promises for pixabay feed handler');
        feeds.forEach(url => {
            models.push( feedMe(url) );
            console.log(url);
        });
    } else {
        _util.traceLog('Pixabay feed is disabled');
    }
