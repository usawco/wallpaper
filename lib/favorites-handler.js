...
var theFeed = ( () => {    
    let models = [];
    _util.infoLog(`Favorites feed enabled? ${config.providers.favorites.enabled}`);
                    ...
                    if ( total > 0 ) {            
                        modelFileObj.items.forEach( entry => {
                            ...
                            try {
                                ...
                                try {                                    
                                    let absPath;
                                    if ( path.isAbsolute( entry.link )) {
                                        absPath = path.resolve(dirPath);            
                                    } else {
                                        absPath = path.resolve(__dirname,'..',entry.link);
                                    }                                    
                                    if ( fs.lstatSync(modelFilePath).isFile ) {
